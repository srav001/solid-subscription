import { dynamicallyExecuteFunction } from './functions/helpers';
import { createSignal} from 'solid-js';

type ValueMutator<SubType> = (val: SubType) => SubType;
type Subscriber<SubType> = (val: SubType) => Promise<void> | void;

type Signal<T> = {
    value: T & ({} | null);
    get(): T & ({} | null);
    set(val: (T & ({} | null)) | ValueMutator<T>): void;
    	/**
		 * A Subscriber(function) is executed when the value is changed.
		 * @param subscriber - type Subscriber = (val: T) => Promise<void> | void;
		 */

    addSub(subscriber: Subscriber<T>): void;
    deleteSub(subscriber: Subscriber<T>): void;
    triggerSubs(): void;
}

type SignalWithMutate<T> = Signal<T> & {
	/**
		 * It mutates the value of the object.
		 * @param mutator - (val: T) => T
		 */
	mutate(mutator: ValueMutator<T>): void
}
	/**
	 * Takes a function as an argument and adds it to the subscriber list.
	 * @param subscriber - Subscriber
	 */
	function addSubscriber<T>(subscriber: Subscriber<T>, subscriptions: Set<Subscriber<T>> ) {
		if (typeof subscriber !== 'function') {
			throw new Error('Subscriber must be a function');
		}
		subscriptions.add(subscriber);
	}
	/**
	 * It deletes an event/subscriber from the subscriptions list.
	 * @param subscriber - Subscriber
	 */
	function deleteSubscriber<T>(subscriber: Subscriber<T>,subscriptions: Set<Subscriber<T>> ) {
		subscriptions.delete(subscriber);
	}
	/**
	 * It loops through the Set of subscribers and executes each function with the value passed in as
	 * an argument
	 * @param {SubType} val - The value that is being passed to the subscribers.
	 */
	function triggerSubscribers<T>(val: T, subscriptions: Set<Subscriber<T>> ) {
		subscriptions.forEach(dep => dynamicallyExecuteFunction(dep, val));
}
	
/**
 * It takes a value and returns an object with a value property that is a shallowRef of the value.
 * passed in, and Subscribers(function) are added to a list to be executed when the value is changed.
 * @param {T} value - T - The initial value of the subscription.
 * @returns A function that returns an object with a shallow reactive value, a subscriber and a
 * few extra methods.
 */
export function useSignal<T, P extends boolean | undefined = undefined>(value: T, primitive?: P ) {
	if (value === undefined) {
		throw new Error('No value provided, Initial value is required');
	}

	const [subGetter, subSetter] = createSignal(value);
	type SubType = ReturnType<typeof subGetter>;

	const _subscriptions: Set<Subscriber<SubType>> = new Set();
	/**
	 * It mutates the $value - for complex types that are typeof object.
	 * @param mutator - (val: T) => T
	 */
	async function mutateSubscriber(mutator: ValueMutator<SubType>) {
		if (typeof mutator !== 'function') {
			throw new Error('Mutation must be a function');
		}
		subSetter(await dynamicallyExecuteFunction(mutator, subGetter()));
		triggerSubscribers(subGetter(), _subscriptions);
	}

	function setValue(val: SubType) {
		// @ts-expect-error not an error maybe?
		subSetter(val); 
		triggerSubscribers(val, _subscriptions);
	}

	const signal = {
		get value(): SubType {
			return subGetter();
		},
		set value(val) {
			setValue(val);
		},

		get(): SubType {
			return subGetter();
		},
		set(val: SubType | ValueMutator<SubType>) {
			// @ts-expect-error same here also ?
			if (typeof val === 'function') setValue(val(subGetter()));
			else setValue(val);
		},

		addSub(subscriber: Subscriber<SubType>) {
			addSubscriber(subscriber, _subscriptions);
		},
		/**
		 * A method that allows you to delete a subscriber from the Set of subscribers.
		 * @param subscriber - Subscriber
		 */
		deleteSub(subscriber: Subscriber<SubType>) {
			deleteSubscriber(subscriber, _subscriptions);
		},
		
	} as P extends undefined | false ? Signal<SubType> : SignalWithMutate<SubType>;

	if(!primitive) {
		(signal as SignalWithMutate<SubType>).mutate = function (mutator: ValueMutator<SubType>): void {
			if (typeof subGetter() !== 'object') {
				throw new Error('Value passed is not an typeof object! Patch only accepts typeof object');
			}
			mutateSubscriber(mutator);
		}

	}

	return signal;
}

